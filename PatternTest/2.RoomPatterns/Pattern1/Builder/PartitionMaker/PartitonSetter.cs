    ...
    partial class RoomP1
    {
        partial class RoomP1Builder
        {
            private class PartitionSetter
            {
                ...
                private static void FindCorrFromSameBaseLast(PartitionParam setterParam)
                {
                    if (setterParam.OriginPost.Type != LineType.Corridor)
                    {
                        setterParam.OriginPost.Point = setterParam.OriginPost.BaseLine.EndPt;
                        return;
                    }

                    double originToCorridorEnd = new Vector3d(setterParam.OriginPost.BaseLine.EndPt - setterParam.OriginPost.Point).Length;
                    ...
                    Point3d baseWithMinCorridor = new Point3d(setterParam.OriginPost.Point + setterParam.OriginPost.BaseLine.UnitTangent * RoomDimension.MinLengthForDoor);
                    setterParam.OriginPost.Point = baseWithMinCorridor;
                    return;
                }
                ...
                private static Boolean IsAvailableOnOtherBase(PartitionParam setterParam, int dividerIndex, int originIndex)
                {
                    ...
                    if (coreSeg[originIndex].Type == LineType.Corridor)
                    {
                        ...
                        if (toStartLength >= RoomDimension.MinLengthForDoor)
                            return true;
                    }
                    ...
                    if (originIndex - dividerIndex > 1)
                    {
                        ...
                        {
                            if ((coreSeg[i].Type == LineType.Corridor) &&
                                (coreSeg[i].Length >= RoomDimension.MinLengthForDoor))
                                return true;
                        }
                    }
